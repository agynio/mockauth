...
test.describe("redirect wildcard policy", () => {
  test("allows configured wildcard types and respects env flag", async ({ request }) => {
    await setAllowAnyRedirect(request, false);
    const tenantId = await createIsolatedTenant(request);
    const clientId = await seedClient(request, tenantId, {
      redirectUris: [
    ...
    await expectRedirectAllowed(request, tenantId, clientId, "https://exact.example.test/callback");
    await expectRedirectAllowed(request, tenantId, clientId, "https://app.example.test/callback/child");
    await expectRedirectAllowed(request, tenantId, clientId, "https://foo.wildcard.test/callback");
    await expectRedirectRejected(request, tenantId, clientId, "https://foo.bar.wildcard.test/callback");

    await expectRedirectRejected(request, tenantId, clientId, "https://anywhere.example.test/logout");

    await setAllowAnyRedirect(request, true);
    await expectRedirectAllowed(request, tenantId, clientId, "https://anywhere.example.test/logout");
...
const createIsolatedTenant = async (request: APIRequestContext) => {
  const response = await request.post("/admin/api/test/seed-tenants-clients", { data: {} });
  expect(response.ok()).toBeTruthy();
  const payload = (await response.json()) as { tenantAId: string };
  if (!payload.tenantAId) {
    throw new Error("Failed to seed tenant for redirect tests");
  }
  return payload.tenantAId;
};
...
const expectRedirectAllowed = async (
  ...
  const response = await authorize(request, tenantId, clientId, redirectUri);
...
const authorize = (request: APIRequestContext, tenantId: string, clientId: string, redirectUri: string) => {
  const url = new URL(`http://127.0.0.1:3000/t/${tenantId}/oidc/authorize`);
